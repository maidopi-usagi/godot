...
// Example placeholder that uses rive runtime types when available.
#if __has_include(<rive/renderer.hpp>) || __has_include("rive/renderer.hpp")
#include <rive/renderer.hpp>
// You can add integration code here once the upstream rive-runtime sources are
// added under thirdparty/rive-runtime and the correct include paths are
// configured.
#endif

// Minimal stubbed API used by Godot integration. Replace with real
// implementations that call into the rive runtime when integrating fully.
...
// A trivial function to demonstrate linkage. Real rendering code will be
// added here later.
...
// Platform helpers exposed for other code to initialize the Metal bridge from
// Godot drivers or modules. These call into the Objective-C++ implementation
// when present.
namespace rive_integration {
...
#if defined(__APPLE__)
	extern void flush_with_command_buffer(void *, void *, uint64_t, uint64_t, uint32_t, uint32_t);
	// Default frame values (1/0) for legacy callers.
	flush_with_command_buffer(cmd_buffer_ptr, nullptr, 1, 0, w, h);
#else
