...
void main() {
	...
	vec3 center_normal = decode_octahedral_normal(center_nd.xy);
	float center_depth = center_nd.z; // Assuming depth is in Z or W? Godot usually packs depth in Z or W of normal buffer? 
	// Wait, u_gbuffer_normal_depth in Godot is usually RG=Normal(Oct), B=Roughness, A=Depth? Or separate depth?
	// In restir_gbuffer.glsl, we wrote:
	// imageStore(normal_depth_img, pos, vec4(normal_oct, 0.0, depth));
	// So Z is depth.
	...
	for (uint i = 0; i < params.neighbor_count; i++) {
		...
		if (neighbor_pos.x >= 0 && neighbor_pos.x < params.screen_size.x && 
			...
			vec3 neighbor_normal = decode_octahedral_normal(neighbor_nd.xy);
			float neighbor_depth = neighbor_nd.z;
