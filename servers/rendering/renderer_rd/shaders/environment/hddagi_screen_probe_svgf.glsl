...
void svgf_temporal_main() {
	...
	if ((params.control.x & 1u) != 0u && current_surface_valid && motion_valid) {
		...
		vec2 previous_uv = half_resolution_specular ? current_uv + motion :
				current_surface_uv + motion + (frame_data.taa_jitter.zw - frame_data.taa_jitter.xy) * 0.5;
...
void svgf_atrous_main() {
	...
	if (specular_mode) {
		...
		float luminance_scale = (params.tuning.w * sqrt(max(center_variance, params.atrous.z)) +
				SPECULAR_MOMENT_LUMINANCE_SCALE * 1e-4) * mix(1.75, 1.0, temporal_confidence);
